...
All commands are terminated with delimiter byte 0x7E.

## Display Specifications
- Display Resolution: Up to 4095x4095 pixels (12-bit addressing)
- Color Values: 0-125 (0x00-0x7D)
...
```
F0 [id1] [id2] [id3] [proto1] [proto2] [command_type] [chunk_count] [seq_number] [command_data...] F7
```
...
- `chunk_count`: Total number of chunks in the complete sequence (1-127)
- `seq_number`: Sequence number of this chunk (0-127)
- `F7`: End of SYSEX
...
- `chunk_count`: Total number of messages in the sequence (1-127)
- `seq_number`: Position of this message in the sequence (0 to chunk_count-1)
...
#### Command Chunking
Multiple drawing commands can be combined into a single SYSEX message to optimize transmission and rendering. Each command within the chunk must be terminated with the command delimiter (0x7E). Commands in a chunk are processed sequentially in the order they appear.

Chunk format:
```
F0 [id1] [id2] [id3] [proto1] [proto2] [command_type] [chunk_count] [seq_number] [command1...] 7E [command2...] 7E [...] F7
```

Example - Draw a rectangle with text inside (single chunk):
```
F0 [id1] [id2] [id3] [proto1] [proto2] 03 01 00  # Single chunk (count=1, seq=0)
   15 00 0A 00 0A 00 32 00 14 01 01 00 00 7E    # Rectangle at (10,10), width=50, height=20, filled
   14 00 0F 00 0F 01 00 4D 45 4E 55 7E           # Text "MENU" at (15,15)
F7
```
...
F0 [id1] [id2] [id3] [proto1] [proto2] 03 02 00
   15 00 0A 00 0A 00 64 00 32 01 01 00 00 7E    # Large rectangle
   14 00 0F 00 0F 01 00 54 49 54 4C 45 7E        # Title text
F7

# Second chunk (count=2, seq=1)
F0 [id1] [id2] [id3] [proto1] [proto2] 03 02 01
   18 00 01 02 00 01 00 7F 4F 4B 7E              # OK button
   16 00 32 00 32 01 01 00 7E                    # Icon
F7
```

Example - Create a button with an icon:
```
F0 [id1] [id2] [id3] [proto1] [proto2] 03
   18 02 00 01 00 01 00 00 53 45 54 7E           # Button labeled "SET" at grid (0,1)
   16 00 01 00 01 02 01 00 7E                    # Icon index 2 next to button
F7
```

When chunking commands:
- Commands are rendered in order, allowing layering effects
- Each command must be properly terminated with 0x7E
...
### Parameter Encoding
Coordinate and size parameters (x, y, width, height, diameter) use 12-bit encoding split across two bytes. Each byte can contain values 0-0x3F (0-63), with 0x7E reserved as a command delimiter:
```
...
### Command Delimiter
Each drawing command in the message must be terminated with the command delimiter byte 0x7E. This byte cannot appear within command parameters. Multiple commands can be sent in a single SYSEX message, each terminated by the delimiter.
...
```
13 [x1_low] [x1_high] [y1_low] [y1_high] [x2_low] [x2_high] [y2_low] [y2_high] [color] [thickness] [line_style] 7E
```
...
- color: Color value (0-125)
- thickness: Line thickness in pixels (1-255)
- line_style: Line style (0=solid, 1=dotted1, 2=dotted2, 3=dotted3)

Example:
Draw a line at y=32, thickness 2:
```
F0 [id1] [id2] [id3] [proto1] [proto2] 03 13 00 00 00 20 07 7F 00 20 01 02 7E F7
```

#### Text Drawing (0x14)
Renders text using built-in bitmap fonts.
```
14 [x_low] [x_high] [y_low] [y_high] [color] [size] [text...] 7E
```
...
```
F0 [id1] [id2] [id3] [proto1] [proto2] 03 14 00 0A 00 0A 01 00 48 65 6C 6C 6F 7E F7
```

#### Rectangle Drawing (0x15)
Draws a filled or outlined rectangle.
```
15 [x_low] [x_high] [y_low] [y_high] [width_low] [width_high] [height_low] [height_high] [color] [line_width] [filled] [line_style] 7E
```
- x, y: Top-left coordinates (12-bit encoded as low 6 bits then high 6 bits, range 0-4095)
...
- color: Color value (0-125)
- line_width: Outline thickness in pixels (1-255, default 1)
...
```
17 [x_low] [x_high] [y_low] [y_high] [diameter_low] [diameter_high] [color] [line_width] [filled] 7E
```
- x, y: Top-left coordinates (12-bit encoded as low 6 bits then high 6 bits, range 0-4095)
...
```
F0 [id1] [id2] [id3] [proto1] [proto2] 03 17 00 1E 00 14 00 14 01 01 01 7E F7
```

#### Icon Drawing (0x16)
Draws a predefined 8x8 bitmap icon. Icons are bitmap patterns stored in the device's memory. The available icons and their corresponding indices are device-specific. If an invalid icon index is specified, the behavior is implementation-defined.
```
16 [x_low] [x_high] [y_low] [y_high] [icon_index] [color] [size] 7E
```
- x, y: Top-left coordinates (12-bit encoded as low 6 bits then high 6 bits, range 0-4095)
- color: Color value (0-125)
- size: Scale factor (0=normal, 1=2x, 2=3x)
...
```
F0 [id1] [id2] [id3] [proto1] [proto2] 03 16 00 14 00 1E 00 01 00 7E F7
```

#### Control Drawing (0x18)
Draws an interactive MIDI control element with dynamic value display.
```
18 [type] [row] [column] [style] [color] [value_low] [value_high] [label...] 7E
```
- type: Control type (0=fader, 1=knob, 2=button, 3=encoder)
- row: Row index in the control grid (0-125)
- column: Column index in the control grid (0-125)
...
- label: UTF-8 encoded control label, terminated by command delimiter (0x7E)

Example:
Draw a vertical fader labeled "Volume" at grid position (1,2) with value at 50%, color 1:
```
F0 [id1] [id2] [id3] [proto1] [proto2] 03 18 00 01 02 00 01 00 7F 56 6F 6C 75 6D 65 7E F7
```
Breakdown:
- type = 0 (fader)
- row = 1
...
4. When processing chunked commands:
   - Each chunk must be properly terminated with the command delimiter (0x7E)
